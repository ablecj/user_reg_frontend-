...
export default function Page() {
  ...
  const [AccIfsc, setAccIfsc] = useState<AccIfscState>({
    ACC: "",
    ifsc: "",
    email: "",
  });
      ...
      <div className="bank_cont">
        ...
        {verificationResult !== null &&
          (verificationResult === "success" ? (
            <span className="bank_success">Bank account verified</span>
          ) : (
            <span className="bank_failed">
              Bank account verification failed
            </span>
          ))}

        {verificationResult && (
          <button className="Next_page_btn" onClick={handleNextBtn}>
            Next
          </button>
        )}
      </div>
