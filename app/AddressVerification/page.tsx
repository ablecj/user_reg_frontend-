...
export default function Page() {
    const [formData, setFormData] = useState<FormData>({ pincode: '', email: '' });
    ...
    const handleVerifyPincode = async (e: React.MouseEvent<HTMLButtonElement>) => {
        e.preventDefault();
    ...
    const handlePostOfficeSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        const selectedOffice = postOffices.find((office) => office.area === selectedArea);
            ...
            <form className='address_container'>
                ...
                <div className='pincode_container'>
                    ...
                    <input
                        ...
                        onChange={(e) => setFormData(prevState => ({ ...prevState, pincode: e.target.value }))}
                ...
                {isVerified ? (
                    <span className='address_verify_sucess'>Address verified successfully</span>
                ) : (
                    <span className='address_verify_failed'>Address verification failed</span>
                )}

                {isVerified && <button className='Addres_next_btn' onClick={handleSubmitBtn}>submit</button>}
            </form>
        </div>
    )
}
